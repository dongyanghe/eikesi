...
export class ChatContent extends React.Component<IProps> {
    ...
    switch (message.MsgType) {
      ...
      case 3:
        ...
        if (uploading) {
          return `
                        <div>
                            <img class="open-image unload" data-id="${message.MsgId}" src="${image.src}" data-fallback="${
            image.fallback
          }" />
  ...
  renderMessages(list, from) {
    return list.data.map((e, index) => {
      ...
      if (
        ...
        return <div key={index} className={classNames('unread', 'message', 'system')} dangerouslySetInnerHTML={{ __html: e.Content }} />;
      }
        ...
        >
          <div>
            ...
            <div className={'content'}>
              <p onContextMenu={this.showMessageAction(message)} dangerouslySetInnerHTML={{ __html: this.getMessageContent(message) }} />
  ...
  showMessageAction = message => () => {
    ...
    if (message.isme && message.CreateTime.getTime() - new Date().getTime() < 2 * 60 * 1000) {
      templates.unshift({
        label: 'Recall',
        click: () => {
      ...
      >
        {user ? (
          <div>
            ...
            <div className={'messages'} onScroll={this.handleScroll} ref={this.viewportRef}>
        ...
        <div className={'tips'} ref={this.tipRef}>
          未读消息
        </div>
      </div>
    );
