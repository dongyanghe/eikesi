...
export class UserInfo extends React.Component<IProps, IState> {
  ...
  render() {
    ...
    if (background) {
      ...
      gradient = `
                -webkit-linear-gradient(top, rgb(${background[0]}, ${background[1]}, ${background[2]}) 5%, rgba(${background[0]}, ${
        background[1]
      }, ${background[2]}, 0) 15%),
                -webkit-linear-gradient(bottom, rgb(${background[0]}, ${background[1]}, ${background[2]}) 5%, rgba(${background[0]}, ${
        background[1]
      }, ${background[2]}, 0) 15%),
                -webkit-linear-gradient(left, rgb(${background[0]}, ${background[1]}, ${background[2]}) 5%, rgba(${background[0]}, ${
        background[1]
      }, ${background[2]}, 0) 15%),
                -webkit-linear-gradient(right, rgb(${background[0]}, ${background[1]}, ${background[2]}) 5%, rgba(${background[0]}, ${
        background[1]
      }, ${background[2]}, 0) 15%)
            `;
    ...
    return (
      <Modal onCancel={this.handleClose} show={this.state.isShow}>
        <ModalBody className={'container'}>
          ...
          >
            ...
            <div className={'username'} dangerouslySetInnerHTML={{ __html: NickName }} />
            ...
            >
              {helper.isChatRoom(UserName) || isFriend ? '发送信息' : '添加好友'}
            </div>
          </div>
...
const mapStateToProps = ({ authentication, applicationProfile, locale }: IRootState) => ({
  ...
  isme: () => authentication.account.user && authentication.account.UserName === authentication.account.User.UserName
});
