...
export const ACTION_TYPES = {
  SEARCH_DIALOGUES: 'dialogue/SEARCH_DIALOGUES',
...
export default (state: DialogueState = initialState, action): DialogueState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_DIALOGUES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_DIALOGUES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_DIALOGUES):
      return {
        ...state,
        loading: false,
...
const apiSearchUrl = 'api/_search/dialogues';

// Actions

export const getSearchEntities: ICrudSearchAction<IDialogue> = query => ({
  type: ACTION_TYPES.SEARCH_DIALOGUES,
  payload: axios.get<IDialogue>(`${apiSearchUrl}?query=` + query)
});
