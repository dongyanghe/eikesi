...
export default class MessageInput extends Component<IProps, IState> {
  ...
  canisend() {
    ...
    if (user.length === 1 && user.slice(-1).pop().UserName === this.props.me.UserName) {
  ...
  async handleEnter(e) {
    ...
    if (!this.canisend() || !message || e.charCode !== 13) return;
    ...
    await Promise.all(
      user.filter(value => value.UserName !== this.props.me.UserName).map(async value => {
        const res = await this.props.sendMessage(
          value,
          {
            content: message,
            type: 1
  ...
  async batchProcess(e) {
    ...
    for (const user of receiver) {
      if (message) {
        await this.props.sendMessage(user, message, true).catch(ex => showMessage(`Send message to ${user.NickName} is failed!`));
        continue;
      }
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={'tips'}>你应该先选择一个联系人。</div>
        <input
          id="messageInput"
        ...
        <div className={'action'}>
          {canisend && <i className="icon-ion-android-attach" id="showUploader" onClick={this.uploaderRef.click()} />}
          {canisend && (
            <i
          ...
          <input
            ...
            type="file"
          />
          <Emoji close={this.toggleEmoji(false)} output={this.writeEmoji} show={this.state.showEmoji} />
        </div>
      </div>
    );
  }
