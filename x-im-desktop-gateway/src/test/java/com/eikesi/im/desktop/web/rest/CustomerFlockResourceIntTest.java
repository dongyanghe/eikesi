package com.eikesi.im.desktop.web.rest;

import com.eikesi.im.desktop.ImGatewayApp;

import com.eikesi.im.desktop.config.SecurityBeanOverrideConfiguration;

import com.eikesi.im.desktop.domain.CustomerFlock;
import com.eikesi.im.desktop.repository.CustomerFlockRepository;
import com.eikesi.im.desktop.repository.search.CustomerFlockSearchRepository;
import com.eikesi.im.desktop.service.CustomerFlockService;
import com.eikesi.im.desktop.service.dto.CustomerFlockDTO;
import com.eikesi.im.desktop.service.mapper.CustomerFlockMapper;
import com.eikesi.im.desktop.web.rest.errors.ExceptionTranslator;
...
import static com.eikesi.im.desktop.web.rest.TestUtil.createFormattingConversionService;
...
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, ImGatewayApp.class})
public class CustomerFlockResourceIntTest {
    ...
    /**
     * This repository is mocked in the com.eikesi.im.desktop.repository.search test package.
     *
     * @see com.eikesi.im.desktop.repository.search.CustomerFlockSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCustomerFlock() throws Exception {
        ...
        // Validate the CustomerFlock in Elasticsearch
        verify(mockCustomerFlockSearchRepository, times(1)).save(testCustomerFlock);
    }
    ...
    public void createCustomerFlockWithExistingId() throws Exception {
        ...
        // Validate the CustomerFlock in Elasticsearch
        verify(mockCustomerFlockSearchRepository, times(0)).save(customerFlock);
    }
    ...
    public void updateCustomerFlock() throws Exception {
        ...
        CustomerFlock updatedCustomerFlock = customerFlockRepository.findById(customerFlock.getId()).get();
    ...
    public void deleteCustomerFlock() throws Exception {
        ...
        // Validate the CustomerFlock in Elasticsearch
        verify(mockCustomerFlockSearchRepository, times(1)).deleteById(customerFlock.getId());
    }

    @Test
    @Transactional
    public void searchCustomerFlock() throws Exception {
        // Initialize the database
        customerFlockRepository.saveAndFlush(customerFlock);
        when(mockCustomerFlockSearchRepository.search(queryStringQuery("id:" + customerFlock.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(customerFlock), PageRequest.of(0, 1), 1));
        // Search the customerFlock
        restCustomerFlockMockMvc.perform(get("/api/_search/customer-flocks?query=id:" + customerFlock.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(customerFlock.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME.toString())))
