...
import com.eikesi.im.desktop.service.FlockRelationService;
...
public class FlockRelationResourceIntTest {
     ...
     *
     * @see com.eikesi.im.desktop.repository.search.FlockRelationSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createFlockRelation() throws Exception {
        ...
        // Validate the FlockRelation in Elasticsearch
        verify(mockFlockRelationSearchRepository, times(1)).save(testFlockRelation);
    }
    ...
    public void createFlockRelationWithExistingId() throws Exception {
        ...
        // Validate the FlockRelation in Elasticsearch
        verify(mockFlockRelationSearchRepository, times(0)).save(flockRelation);
    }
    ...
    public void updateFlockRelation() throws Exception {
        ...
        FlockRelation updatedFlockRelation = flockRelationRepository.findById(flockRelation.getId()).get();
    ...
    public void deleteFlockRelation() throws Exception {
        ...
        // Validate the FlockRelation in Elasticsearch
        verify(mockFlockRelationSearchRepository, times(1)).deleteById(flockRelation.getId());
    }

    @Test
    @Transactional
    public void searchFlockRelation() throws Exception {
        // Initialize the database
        flockRelationRepository.saveAndFlush(flockRelation);
        when(mockFlockRelationSearchRepository.search(queryStringQuery("id:" + flockRelation.getId())))
            .thenReturn(Collections.singletonList(flockRelation));
        // Search the flockRelation
        restFlockRelationMockMvc.perform(get("/api/_search/flock-relations?query=id:" + flockRelation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(flockRelation.getId().intValue())))
            .andExpect(jsonPath("$.[*].remarkName").value(hasItem(DEFAULT_REMARK_NAME.toString())))
