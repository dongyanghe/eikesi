...
public class CustomerFlockResource {
    ...
    public ResponseEntity<CustomerFlockDTO> getCustomerFlock(@PathVariable Long id) {
        ...
        Optional<CustomerFlockDTO> customerFlockDTO = customerFlockService.findOne(id);
        return ResponseUtil.wrapOrNotFound(customerFlockDTO);
    }
    ...
    /**
     * SEARCH  /_search/customer-flocks?query=:query : search for the customerFlock corresponding
     * to the query.
     *
     * @param query the query of the customerFlock search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/customer-flocks")
    @Timed
    public ResponseEntity<List<CustomerFlockDTO>> searchCustomerFlocks(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of CustomerFlocks for query {}", query);
        Page<CustomerFlockDTO> page = customerFlockService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/customer-flocks");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
