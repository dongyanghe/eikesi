...
import com.eikesi.im.desktop.service.HistoryMessageService;
import com.eikesi.im.desktop.web.rest.errors.BadRequestAlertException;
import com.eikesi.im.desktop.web.rest.util.HeaderUtil;
import com.eikesi.im.desktop.web.rest.util.PaginationUtil;
...
public class HistoryMessageResource {
    ...
    public ResponseEntity<HistoryMessageDTO> getHistoryMessage(@PathVariable Long id) {
        ...
        Optional<HistoryMessageDTO> historyMessageDTO = historyMessageService.findOne(id);
        return ResponseUtil.wrapOrNotFound(historyMessageDTO);
    }
    ...
    /**
     * SEARCH  /_search/history-messages?query=:query : search for the historyMessage corresponding
     * to the query.
     *
     * @param query the query of the historyMessage search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/history-messages")
    @Timed
    public ResponseEntity<List<HistoryMessageDTO>> searchHistoryMessages(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of HistoryMessages for query {}", query);
        Page<HistoryMessageDTO> page = historyMessageService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/history-messages");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
