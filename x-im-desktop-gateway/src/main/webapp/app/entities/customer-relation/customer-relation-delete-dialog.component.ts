...
export class CustomerRelationDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ customerRelation }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(CustomerRelationDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
                this.ngbModalRef.componentInstance.customerRelation = customerRelation;
