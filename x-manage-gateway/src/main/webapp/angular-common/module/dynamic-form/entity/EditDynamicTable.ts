...
export class EditDynamicTable extends  FormControlBase<Array<any>> {
  ...
  constructor(label: string, placeholder, keys: string | Array<string>, value: Array<any>, theadColList: Array<TheadCol>, editDynamicTableColList: Array<EditDynamicTableCol>, required: boolean = false, readonly: boolean = false, disabled: boolean = false, minlength?: number, maxlength?: number, title?: string, order?: number, formItemColSize?: FormItemColSize) {
    super('editDynamicTable', label, placeholder, keys, value, null, required, readonly, disabled, undefined, minlength, maxlength, title, order, formItemColSize);
    const self = this;
    self.value = value;
    ...
    if (this.value && this.value.length) {
      for (let obj of this.value) {
        ...
        for (let editDynamicTableCol of this.editDynamicTableColList) {
          let validatorOrOpts = new Array(Validators.min(editDynamicTableCol.formControl.minlength),
          ...
          editDynamicTableGroup[editDynamicTableCol.formControl.key]
            = new FormControl(obj[editDynamicTableCol.formControl.key], validatorOrOpts);
        }
  ...
  addRow(formGroup: FormGroup) {
    ...
    for (let editDynamicTableCol of self.editDynamicTableColList) {
      let validatorOrOpts = new Array(Validators.min(editDynamicTableCol.formControl.minlength), Validators.max(editDynamicTableCol.formControl.maxlength));
      if (editDynamicTableCol.formControl.required) {
        validatorOrOpts.push(Validators.required);
      }
      editDynamicTableGroup[editDynamicTableCol.formControl.key] = new FormControl(editDynamicTableCol.formControl.value, validatorOrOpts);
    }
