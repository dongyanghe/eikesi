    ...
    switch (message.MsgType) {
        ...
        case 10000:
            ...
            if (helper.isChatRoom(userid)) {
                let user = await customerRelation.getUser(userid);
...
async function updateMenus({ conversations = [], customerRelation = [] }) {
    ipcRenderer.send('menu-update', {
        ...
        customerRelation: customerRelation.map(e => ({
            id: e.UserName,
            name: e.RemarkName || e.NickName,
            avatar: e.HeadImgUrl,
        })),
...
class Chat {
    ...
    @action async loadChats(chatSet) {
        ...
        updateMenus({
            ...
            customerRelation: customerRelation.memberList.filter(e => helper.isContact(e)),
        });
        return res;
    }
    ...
    @action async addMessage(message, sync = false) {
        ...
        if (sync) {
            ...
            user = customerRelation.memberList.find(e => e.UserName === from);

            message.isme = true;
            message.HeadImgUrl = authentication.user.User.HeadImgUrl;
    ...
    @action async upload(file, user = self.user) {
        ...
        const process = async(index) => {
            ...
            formdata.append('uploadmediarequest', JSON.stringify(Object.assign({
                ...
                ClientMediaId: id,
                DataLen: file.size,
                FromUserName: authentication.user.User.UserName,
